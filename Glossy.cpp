...
float3 Glossy::shade(const Ray& r, HitInfo& hit, bool emit) const
{
  ...
  //       the shader for the surface it hit.
    ...
    float3 transparentShade = R*shade_new_ray(reflected, hit_reflected) + (1.0f - R)*shade_new_ray(refracted, hit_refracted);
    
    return transparentShade + R * Phong::shade(r, hit, emit);
}
